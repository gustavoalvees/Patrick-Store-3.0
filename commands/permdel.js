...
module.exports = {
    ...
    run: async(client, message, args) => {
      ...
      if(user !== `${perms.get(`${user}_id`)}`) return message.reply(`<:error:1069750943417122846> | Essa pessoa não tem permissão ainda!`).then(msg => setTimeout(() => msg.delete().catch(err => console.log(err)), 5000));
      if(isNaN(args)) return message.reply(`<:error:1069750943417122846> | Você só pode adicionar IDs!`)
        
      message.reply(`<:concluido:1046917987673853992> | Usuário removido!`)
